...
- **[z-go-pgvector](./z-go-pgvector/SKILL.md)** — pgvector-backed similarity search in Go — declaring vector(N) columns, encoding []float32 via the pgvector-go client (pgvector.NewVector), distance operators (<->, <=>, <#>),…
...
- **[z-go-telegram-bot](./z-go-telegram-bot/SKILL.md)** — Hand-rolled Telegram Bot API client in Go for MarkdownV2/Mini-App/Stars-level control. Covers the minimal Bot struct over net/http, common methods (sendMessage, sendPhoto, answerCallbackQuery)…
